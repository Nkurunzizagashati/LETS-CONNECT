                    ...
                    <div class=" flex flex-col items-center justify-center  gap-8">
                        ...
                        <div class=" flex flex-col gap-6 w-[80%]">
                            <div class=" flex flex-col items-center justify-center gap-4">
                                <img class=" w-[60%] h-auto object-cover rounded-lg border"
                                    src="{{ asset($post->file_path) }}" />
                                <div class=" flex flex-col gap-4">
                                    <p class=" max-w-[600px] border p-4"><span
                                            class=" text-gray-700 font-bold">Description</span>:
                                        {{ $post->description }}</p>
                                    <p class=" h-8 w-80 truncate"><span
                                            class=" text-gray-700 font-bold">Category</span>: {{ $post->post_category }}
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class=" flex items-center justify-center py-2">
                        ...
                        @if ($post->user->id === Auth::user()->id)
                            <button wire:click="deletePost({{ $post }})"
                                class=" text-white border rounded-lg bg-red-600 px-4 py-2 hover:bg-red-500 ">DeletePost</button>
                        @endif
                    </div>
